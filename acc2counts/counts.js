...
const B2 = [0.0753349039750657, 0.0, -0.3013396159002628, 0.0, 0.4520094238503942, 0.0, -0.3013396159002628, 0.0, 0.0753349039750657]
const A2 = [1.0, -4.2575497111306, 7.543176557521139, -7.64673626503976, 5.33187310787808, -2.7027389238066353, 0.8452545660100996, -0.1323148049950936, 0.019035473586069288]

// Filter initial response
const zi = [-0.07532883864659122, -0.0753546620840857, 0.22603070565973946, 0.22598432569253424, -0.22599275859607648, -0.22600915159543014, 0.07533559105142006, 0.07533478851866574, 0]
...
const odd_ext = (x, n) => {
  const left = x.slice(0, n).reverse().map(d => 2 * x[0] - d)
  const right = x.slice(-n).reverse().map(d => 2 * x[x.length-1] - d)
...
const getCounts = (values) => {
  ...
  const fx8up = lfilter(B.map(x => x * gain), A, filtered)

  const fx8 = fx8up
    .filter((_, i) => i % 3 === 0)
    .map(d => d < -peakThreshold ? -peakThreshold : d > peakThreshold ? peakThreshold : d)

  const truncated = fx8.map(d => Math.abs(d) < deadband ? 0 : Math.floor(Math.abs(d)/adcResolution))
